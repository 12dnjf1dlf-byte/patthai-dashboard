    ...
    <div className="rounded-2xl p-6" style={{ backgroundColor: "#1C1E2E" }}>
      ...
      <ResponsiveContainer width="100%" height={300} style={{ overflow: "visible" }}>
        <BarChart data={data} margin={{ top: 32, right: 4, left: 0, bottom: 0 }} barCategoryGap="20%">
          ...
          <YAxis tickFormatter={formatKRWShort} tick={{ fill: "rgba(255,255,255,0.5)", fontSize: 11 }} axisLine={false} tickLine={false} width={60} />
          <Tooltip
          ...
          <Bar dataKey="2025" fill="#7B70EE" radius={[4, 4, 0, 0]} barSize={16}>
            <LabelList dataKey="2025" position="top" formatter={(v: unknown) => formatKRWShort(Number(v))} style={{ fill: "rgba(255,255,255,0.6)", fontSize: 10 }} />
          </Bar>
          <Bar dataKey="2026" fill="#00CFAA" radius={[4, 4, 0, 0]} barSize={16}>
            <LabelList dataKey="2026" position="top" formatter={(v: unknown) => formatKRWShort(Number(v))} style={{ fill: "rgba(255,255,255,0.6)", fontSize: 10 }} />
