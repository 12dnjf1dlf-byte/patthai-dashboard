import { getNutralabData, getNutralabAdCost, NamyuRow, AdCostRow } from "@/lib/sheets";
...
function processRaw(rows: NamyuRow[], adCosts: AdCostRow[]) {
  ...
  adCosts.forEach((r) => { const m = parseAdMonth(r.월); if (m) adCostMap[m] = r.광고비; });
  ...
  rows.forEach((r) => { catMap[r.카테고리] = (catMap[r.카테고리] ?? 0) + r.매출; });
  const categoryData = Object.entries(catMap).sort(([, a], [, b]) => b - a).map(([category, value]) => ({ category, value }));
...
export default async function NutralabPage() {
  let rows: NamyuRow[] = [];
    ...
    <main className="min-h-screen p-6 md:p-10" style={{ backgroundColor: "#13141F" }}>
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold" style={{ color: "rgba(255,255,255,0.87)" }}>뉴트라랩 쿠팡 매출 현황</h1>
        </div>
      </div>
      ...
      <div className="mb-6"><CoupangMonthlyChart data={monthlyData} /></div>
      <div className="mb-6"><CategoryPie data={categoryData} /></div>
